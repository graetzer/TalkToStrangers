...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('send_area_msg', function (msg) {
        ...
        if (msg_queue.length > MAX_MSG_QUEUE_LEN)
            msg_queue.pop();

        var near_clients = clients.filter(function (val) {
            return distance([val.gps, clients[socket.id].gps]) <= clients[socket.id].max_dist;
        });

        if (MAX_RECEIVER < near_clients.length)
            clients[socket.id].dec_max_dist();

        near_clients.forEach(function (val) {
...
var ChatUser = (function () {
    function ChatUser(id, gps) {
        this.max_dist = 10;
        this.id = id;
        this.gps = gps;
    }
    ChatUser.prototype.dec_max_dist = function () {
...
var Message = (function () {
    function Message(text, gps, timestamp) {
        this.text = text;
        this.gps = gps;
